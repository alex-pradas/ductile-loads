...
class LoadSet(BaseModel):
    ...
    def to_json(self, file_path: str | PathLike | None = None, indent: int = 2) -> str:
        """
        Write LoadSet to a JSON file and/or return JSON string.

        Args:
            file_path: Optional path to write the JSON file. If None, only returns string.
            indent: JSON indentation level (default: 2)

        Returns:
            str: JSON representation of the LoadSet
        ...
        try:
            ...
            raise ValueError(f"Failed to serialize LoadSet to JSON: {e}")
    ...
    def convert_to(self, units: ForceUnit) -> "LoadSet":
        ...
        moment_factors = {"Nmm": 0.001, "Nm": 1.0, "kNm": 1000.0, "klbs.in": 112.9848293}
    ...
    def to_ansys(
        ...
        Args:
            folder_path: Directory to save the files. Defaults to 'temp'.
            name_stem: Optional base name for the files. If None, uses only load case names.
            exclude: List of point names to exclude from export.
        ...
        for load_case in self.load_cases:
            ...
            ansys_content = self._generate_ansys_content(load_case, exclude=exclude)
    ...
    def _generate_ansys_content(
        ...
        for point_load in load_case.point_loads:
            ...
            # Collect non-zero components and write grouped by point
            non_zero = [(dof, value) for dof, value in components if value != 0.0]

            if non_zero:
                lines.append(f"cmsel,s,{pilot_name}")
                for dof, value in non_zero:
    ...
    def get_point_extremes(self, output: PathLike | None = None) -> dict:
        """
        Get extreme values (max/min) for each point and component across all load cases.

        Args:
            output: Optional file path. If given, the extremes are also
                written as JSON to this file.
        ...
        if output is not None:
            Path(output).write_text(json.dumps(filtered_extremes, indent=2))
    ...
    @staticmethod
    def _format_value_plain(value: float) -> str:
        """Format a numeric value as plain text (no Rich markup)."""
        if value == 0.0:
            return "0.000"
        elif abs(value) >= 10000 or abs(value) < 0.01:
            return f"{value:.3e}"
    ...
    def envelope_to_markdown(self, output: PathLike | None = None) -> str:
        ...
        name = self.name or "Unnamed LoadSet"
        lines: list[str] = [
            f"## {name} — Envelope Summary",
            "",
            f"Version: {self.version} | Units: {self.units.forces}, {self.units.moments}",
            "",
        ]

        extremes = self.get_point_extremes()
        if not extremes:
            lines.append("_No extreme values found (all zero)._")
            return "\n".join(lines)

        components = ["fx", "fy", "fz", "mx", "my", "mz"]
        header = "| Point | Type | " + " | ".join(c.upper() for c in components) + " |"
        sep = "|" + "|".join(["-------", "------"] + ["-------:"] * 6) + "|"
        lines.append(header)
        lines.append(sep)

        fmt = self._format_value_plain
        for point_name in sorted(extremes.keys()):
            point_data = extremes[point_name]

            max_cells = [
                fmt(point_data[c]["max"]["value"]) if c in point_data else "0.000"
                for c in components
            ]
            lines.append(
                f"| {point_name} | max | " + " | ".join(max_cells) + " |"
            )

            min_cells = [
                fmt(point_data[c]["min"]["value"]) if c in point_data else "0.000"
                for c in components
            ]
            lines.append(
                f"| | min | " + " | ".join(min_cells) + " |"
            )

        lines.append("")
        n_cases = len(self.load_cases)
        lines.append(f"Points: {len(extremes)} | From {n_cases} load cases")
    ...
    def print_envelope(self) -> None:
        """
        Print envelope summary in wide format with one max and one min row per point.

        Each row shows values for all 6 force/moment components. The max row shows
        the maximum value found across all load cases for each component independently,
        and the min row shows the minimum.
        """
        from rich.console import Console
        from rich.table import Table
        from rich.panel import Panel
        ...
        # Header panel with metadata
        name = self.name or "Unnamed LoadSet"
        header_text = f"[bold cyan]{name}[/bold cyan] - Envelope Summary\n"
        header_text += f"Version: {self.version} | "
        header_text += f"Units: {self.units.forces}, {self.units.moments}"
        console.print(Panel(header_text, title="LoadSet Envelope", border_style="cyan"))
        ...
        table.add_column("Type")
        for comp in components:
            table.add_column(comp.upper(), justify="right")
        ...
        for point_name in sorted(extremes.keys()):
            ...
            max_cells = []
            for comp in components:
                if comp in point_data:
                    val = point_data[comp]["max"]["value"]
                    max_cells.append(f"[bold green]{self._format_value(val)}[/bold green]")
                else:
                    max_cells.append("[dim]0.000[/dim]")
            ...
            for comp in components:
                if comp in point_data:
                    val = point_data[comp]["min"]["value"]
                    min_cells.append(f"[bold red]{self._format_value(val)}[/bold red]")
                else:
                    min_cells.append("[dim]0.000[/dim]")
